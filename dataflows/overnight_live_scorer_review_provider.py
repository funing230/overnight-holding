...
def build_scorer_review_payload(
    ...
    return f"""
你是 TradingAgents2.0 的 A 股一夜持股法尾盘评分器。
...
def extract_scorer_review_json(text: str) -> dict[str, Any]:
    m = re.search(r"SCORER_REVIEW_JSON_START\s*(\{.*?\})\s*SCORER_REVIEW_JSON_END", text, re.S)
    if not m:
        raise ValueError("SCORER_REVIEW_JSON block not found")
    return json.loads(m.group(1))


def neutral_scorer_scores(candidate_pool: pd.DataFrame, reason: str = "neutral_scorer_fallback") -> pd.DataFrame:
    rows = []
...
def parse_scorer_scores(text: str, candidate_pool: pd.DataFrame) -> ScorerReviewParseResult:
    try:
        data = extract_scorer_review_json(text)
        ...
        merged.loc[missing, "agent_reason"] = "missing_from_scorer_json_neutral"
        return ScorerReviewParseResult(merged[REVIEW_REQUIRED_COLUMNS], parsed=True)
    except Exception as exc:
        return ScorerReviewParseResult(neutral_scorer_scores(candidate_pool, reason=f"parse_failed: {exc}"), parsed=False, error=str(exc))


def write_scorer_artifacts(
    out_dir: str | Path,
    ...
    parse_result = parse_scorer_scores(decision_text, candidate_pool)
    paths = {
        "scorer_review_report": out / "agent_review_report.md",
        "scorer_review_scores": out / "agent_review_scores.csv",
        "scorer_review_state": out / "agent_review_state.json",
        "scorer_review_parse_manifest": out / "agent_review_parse_manifest.json",
    }
    paths["scorer_review_report"].write_text(decision_text + "\n", encoding="utf-8")
    parse_result.scores.to_csv(paths["scorer_review_scores"], index=False)
    paths["scorer_review_state"].write_text(json.dumps(final_state, ensure_ascii=False, indent=2, default=str) + "\n", encoding="utf-8")
    paths["scorer_review_parse_manifest"].write_text(
