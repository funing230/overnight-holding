...
"""Selector (TradingAgents) review helpers for Top50 -> Top15 overnight live funnel.

Selector review runs before the 14:30 light review.  It should be earlier and more
...
SELECTOR_REVIEW_REQUIRED_COLUMNS = [
    "ts_code",
    "heavy_score",
    "heavy_tier",
    "heavy_veto",
...
@dataclass
class SelectorReviewParseResult:
    scores: pd.DataFrame
    parsed: bool
    error: str = ""


def build_selector_review_payload(
    ...
    """Build selector review prompt for Top50 -> Top15 research compression."""
    ...
    return f"""
你是 TradingAgents2.0 的 A 股一夜持股法筛选器（候选池研究器）。

目标：现在不是最终 14:55 出票，也不是评分器尾盘复核；你的任务是在更早时间把 deterministic Top{top_k} 候选池压缩成高质量 Top{target_top_n} 研究池，供后续 14:30 scorer review 和 14:55 最终融合使用。
...
- 不要在 JSON 之前输出任何文字。
- top_picks 最多保留 {target_top_n} 条，按 selector_keep_rank 从 1 开始排序。
- rejects 只写明确不应进入研究池/应 veto 的股票。
- 未出现在 top_picks 或 rejects 的股票，程序会自动视为 watch；不要重复枚举。
- selector_tier=reject 或 selector_veto=true 的股票原则上不进入 Top{target_top_n}。
- 如果无法确认，宁可不写入 top_picks/rejects，也不要编造事实。
- 为节省输出长度，优先保持 JSON 完整，summary 宁可极短。
""".strip()


def extract_selector_review_json(text: str) -> dict[str, Any]:
    m = re.search(r"SELECTOR_REVIEW_JSON_START\s*(\{.*?\})\s*SELECTOR_REVIEW_JSON_END", text, re.S)
    if m:
        return json.loads(m.group(1))

    start = re.search(r"SELECTOR_REVIEW_JSON_START\s*", text, re.S)
    if not start:
        raise ValueError("SELECTOR_REVIEW_JSON block not found")

    tail = text[start.end():]
    first_brace = tail.find("{")
    if first_brace < 0:
        raise ValueError("SELECTOR_REVIEW_JSON start found but JSON body missing")
    ...
    try:
        ...
        if repaired is None:
            raise ValueError("SELECTOR_REVIEW_JSON start found but JSON appears truncated")
...
def neutral_selector_review_scores(candidate_pool: pd.DataFrame, reason: str = "neutral_selector_fallback") -> pd.DataFrame:
    rows = []
    ...
    return pd.DataFrame(rows, columns=SELECTOR_REVIEW_REQUIRED_COLUMNS)


def _fallback_selector_scores_from_ranked_text(text: str, candidate_pool: pd.DataFrame) -> pd.DataFrame | None:
...
def _normalize_selector_review_records(records: pd.DataFrame, default_start_rank: int = 1) -> pd.DataFrame:
    if records.empty:
        return pd.DataFrame(columns=SELECTOR_REVIEW_REQUIRED_COLUMNS)
    reviews = records.copy()
    for col in SELECTOR_REVIEW_REQUIRED_COLUMNS:
        if col not in reviews.columns:
            if col == "heavy_veto":
                ...
                reviews[col] = ""
    reviews = reviews[SELECTOR_REVIEW_REQUIRED_COLUMNS].copy()
...
def _materialize_schema_scores(data: dict[str, Any], candidate_pool: pd.DataFrame, target_top_n: int | None = None) -> pd.DataFrame:
    ...
    if isinstance(data.get("reviews"), list):
        reviews = _normalize_selector_review_records(pd.DataFrame(data.get("reviews", [])))
        ...
        merged.loc[missing, "heavy_reason"] = "missing_from_selector_json_neutral"
        ...
        return merged[SELECTOR_REVIEW_REQUIRED_COLUMNS]

    top_picks = _normalize_selector_review_records(pd.DataFrame(data.get("top_picks", [])), default_start_rank=1)
    rejects = _normalize_selector_review_records(pd.DataFrame(data.get("rejects", [])), default_start_rank=999)
    ...
    explicit = pd.concat([top_picks, rejects], ignore_index=True) if (not top_picks.empty or not rejects.empty) else pd.DataFrame(columns=SELECTOR_REVIEW_REQUIRED_COLUMNS)
...
def parse_selector_review_scores(text: str, candidate_pool: pd.DataFrame) -> SelectorReviewParseResult:
    try:
        data = extract_selector_review_json(text)
        scores = _materialize_schema_scores(data, candidate_pool, target_top_n=int(data.get("target_top_n") or 15))
        return SelectorReviewParseResult(scores, parsed=True)
    except Exception as exc:
        fallback = _fallback_selector_scores_from_ranked_text(text, candidate_pool)
        if fallback is not None:
            return SelectorReviewParseResult(fallback, parsed=True, error=f"json_parse_failed_used_ranked_text_fallback: {exc}")
        return SelectorReviewParseResult(neutral_selector_review_scores(candidate_pool, reason=f"parse_failed: {exc}"), parsed=False, error=str(exc))


def apply_selector_selection(candidate_pool: pd.DataFrame, scores: pd.DataFrame, target_top_n: int = 15) -> pd.DataFrame:
    ...
    out["selector_selection_score"] = 0.55 * base_live + 0.45 * out["heavy_score"] + out["heavy_adjustment"] + tier_bonus + rank_bonus
    out.loc[out["heavy_veto"] | out["heavy_tier"].eq("reject"), "selector_selection_score"] = -999.0
    out["rank_in_selector_review"] = out["selector_selection_score"].rank(method="first", ascending=False)
    selected = out.sort_values(["rank_in_selector_review", "ts_code"]).head(target_top_n).copy()
    return selected.reset_index(drop=True)


def write_selector_review_artifacts(
    out_dir: str | Path,
    ...
    parse_result = parse_selector_review_scores(decision_text, candidate_pool)
    selected = apply_selector_selection(candidate_pool, parse_result.scores, target_top_n=target_top_n)
    paths = {
        "selector_review_report": out / "selector_review_report.md",
        "selector_review_scores": out / "selector_review_scores.csv",
        "selector_selected_top15": out / "selector_selected_top15.csv",
        "selector_review_state": out / "selector_review_state.json",
        "selector_review_parse_manifest": out / "selector_review_parse_manifest.json",
    }
    paths["selector_review_report"].write_text(decision_text + "\n", encoding="utf-8")
    parse_result.scores.to_csv(paths["selector_review_scores"], index=False)
    selected.to_csv(paths["selector_selected_top15"], index=False)
    paths["selector_review_state"].write_text(json.dumps(final_state, ensure_ascii=False, indent=2, default=str) + "\n", encoding="utf-8")
    paths["selector_review_parse_manifest"].write_text(
